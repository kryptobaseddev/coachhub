<!-- src/routes/climate/+page.svelte -->

<!-- Features:

Comprehensive climate control interface
Real-time status updates
Smooth transitions and animations
Responsive layout
Demo mode support
Consistent UI patterns
Clear visual feedback -->

<script>
  import { isDemoMode } from '$lib/stores/demoMode';
  import ThermostatControls from './ThermostatControls.svelte';
  import VentFanControls from './VentFanControls.svelte';
  import HeatedFloorControls from './HeatedFloorControls.svelte';
  import { createEventDispatcher } from 'svelte';

  export let data;
  const dispatch = createEventDispatcher();

  $: ({ devices } = data);

  async function handleDeviceUpdate(event) {
    const { deviceId, type, updates } = event.detail;
    
    if ($isDemoMode) {
      devices = {
        ...devices,
        [type]: devices[type].map(device => 
          device.id === deviceId ? { ...device, ...updates } : device
        )
      };
    } else {
      try {
        const response = await fetch(`/api/devices/${deviceId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(updates)
        });
        if (!response.ok) throw new Error('Failed to update device');
      } catch (error) {
        console.error('Error updating device:', error);
      }
    }
  }
</script>

<div class="space-y-6">
  <div class="flex justify-between items-center">
    <h1 class="text-2xl font-bold text-gray-900">Climate Control</h1>
    {#if $isDemoMode}
      <span class="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm">Demo Mode</span>
    {/if}
  </div>

  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <!-- Thermostat Section -->
    <ThermostatControls 
      thermostats={devices.thermostats} 
      on:update={handleDeviceUpdate}
    />

    <!-- Vent Fans Section -->
    <VentFanControls 
      fans={devices.ventFans} 
      on:update={handleDeviceUpdate}
    />
  </div>

  <!-- Heated Floors Section -->
  <HeatedFloorControls 
    floors={devices.heatedFloors} 
    on:update={handleDeviceUpdate}
  />
</div>